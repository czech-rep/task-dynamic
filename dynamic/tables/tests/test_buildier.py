...
class TestBuildier(TestCase):
    def setUp(self):
        self.table = Table.objects.create(name='test_table')
        self.fields = [
            Field.objects.create(
                table=self.table,
                name="".join(random.choice(string.ascii_lowercase) for _ in range(5)),
                field_type=random.choice([0, 1, 2])
            ) for _ in range(4)
        ]
    ...
    def test_tables_model(self):
        ...
        self.assertEqual(self.model.__name__, 'dynamic_table_' + self.table.name)
    ...
    def test_change_name(self):
        ...
        self.assertEqual(changes[0][0], 'alter_db_table')

        table_build.alter_table(self.model, changes)

        self.assertEqual(self.model._meta.db_table, 'dynamic_table_aa')
